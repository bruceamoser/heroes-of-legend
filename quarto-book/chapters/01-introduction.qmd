...
This keeps combat fast, cinematic, and dangerous. You can't stack armor so high that goblins can't touch you. Every attack lands. Armor reduces damage instead of preventing hits (see `@sec-chapter-armor-shields`{=typst}). The knight in plate mail still gets knocked around, they just stay standing longer. Combat becomes about who's left standing, not about who finally rolled high enough to participate.
...
Full armor rules: `@sec-chapter-armor-shields`{=typst}.
:::
...
*Kael rolls:* "Four, three, five. That's twelve. My **Brawn** is +2, and I've got Blades Fighting at **Adept** (see `@sec-chapter-skills`{=typst}), so that's another +2. Total is sixteen."
...
Adept and Master spells have per-encounter and per-session limits (see `@sec-chapter-magic-system`{=typst}), not because we want you counting gas, but because Master-tier effects reshape the battlefield. Those limits are about spotlight management, not resource attrition. When you unleash *Volcanic Eruption*, it should be a moment everyone remembers, not something you do three times before lunch.
...
Two more things worth understanding. **Flat damage** means one roll tells you everything: no separate damage dice, no arithmetic slowdown. The combat chapter (`@sec-chapter-combat`{=typst}) covers this in full. **Disciplines** mean no class-locked skills: anyone can learn anything, it just costs more outside your wheelhouse. The disciplines chapter (`@sec-chapter-disciplines`{=typst}) explains the full system.
...
::: {.callout-tip}
## Ready to Begin?
- **New player?** Turn to ``@sec-chapter-character-creation`{=typst}`{=typst} and build your first hero.
- **New GM?** Skim `@sec-chapter-attributes`{=typst} through `@sec-chapter-disciplines`{=typst}, then dig into `@sec-chapter-gm-guidance`{=typst}.

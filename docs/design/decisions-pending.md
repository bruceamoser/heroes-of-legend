...
| 13 | 15-equipment | Mount-control rules keyed to Animal Handling, which #256 removed from the skill roster. Re-keyed to Athletics (mount control = physical exertion; Survival/Nature fit outdoors but not battle control). Implemented default 2026-08-14 — veto to revert. | Council (Architect) | implemented default 2026-08-14 | Re-keyed: warhorse, Controlling a Mount, Fighting from a Moving Mount all use Athletics (PR #275). Also re-keyed livestock roundup check to Survival or Nature (ch19) and disguise-seeing to Reason (Insight) (ch20). |
| 14 | 07-skills | ch07 header claims "The 18 Skills" but the skill table lists 20 rows (Physical 4, Knowledge 6, Social 4, Subterfuge 4, Craft 2). Count wording needs reconciliation — either drop two skills (which two?) or update the header to 20. | Council (EIC, flagged for ch07's day) | open | |

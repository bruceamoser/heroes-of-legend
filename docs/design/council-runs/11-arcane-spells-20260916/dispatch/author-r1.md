...
2. **Your round-1 packet**: `/home/bmoser/.hermes/councils/hol-rulebook/runs/20260916-2002/briefs/round-01/author.json`
...
```
/tmp/hol-council/11-arcane-spells/findings/author-r1.json
```
...
**Your role card:** `/home/bmoser/repos/synod/references/roles/hol/author.md`

You are the CONSISTENCY AND VOICE lens. Aim hardest at: (a) NAME COLLISIONS, the duplicated ward ladder (Bark Skin / Stone Skin / Iron Skin appear in this chapter AND in `12-divine-spells.qmd` with identical content, while the primal twin of the same ladder at `12:178-181` was given its own three names) - and any OTHER card name in this file that also appears elsewhere in the book (run a book-wide sweep, do not guess); (b) the `*Keywords:*` vocabulary, which uses many more tags than the book names at `19:560`, and prints paired tags in inconsistent order; (c) section introductions and promises against the cards that follow them, including the one-card Plants section; (d) tonal drift, repeated phrasing and flavour-vs-rules balance in the 53 spell cards.

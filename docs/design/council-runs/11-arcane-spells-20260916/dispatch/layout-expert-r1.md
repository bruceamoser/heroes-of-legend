...
2. **Your round-1 packet**:
   `/home/bmoser/.hermes/councils/hol-rulebook/runs/20260916-2002/briefs/round-01/layout-expert.json`
...
```
/tmp/hol-council/11-arcane-spells/findings/r1/layout-expert-r1-<id>.json
```
...
## YOUR LENS — layout-expert

**Your role card:** `/home/bmoser/repos/synod/references/roles/hol/layout-expert.md`

You are the PRESENTATION lens. Aim hardest at: (a) card layout consistency across all 68 cards - the header line, the outcome block, field order, and the three cards that carry a single Effect line against the rest; (b) whether the tier printed in every heading reads unambiguously for a card whose outcome block must also print its row; (c) any structure that would render badly in Typst - the file is native Typst, one {=typst} fence, a label on the line after its heading, no markdown bold, zero em-dashes; (d) the section headings and the singleton sections, and whether they read as deliberate or as an accident of the last edit wave. You cannot open a rendered PDF: say plainly which of your findings would need one to settle, and which are decidable from the source structure alone.
